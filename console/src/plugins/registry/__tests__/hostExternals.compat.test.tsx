 ...
 *   window.Cognitwin.registerRoutes("cloudpaw", [{ path: "/a2a", ... }])
 *
...
describe("legacy registerRoutes shim", () => {
  it("attaches window.Cognitwin.{menu, route, slot, audit}", () => {
    expect(typeof window.Cognitwin.menu?.add).toBe("function");
    expect(typeof window.Cognitwin.route?.add).toBe("function");
    expect(typeof window.Cognitwin.slot?.fill).toBe("function");
    expect(typeof window.Cognitwin.audit?.overrides).toBe("function");
  });

  it("translates CloudPaw-shape registerRoutes into menu+route", () => {
    window.Cognitwin.registerRoutes!("cloudpaw", [
      {
        path: "/a2a",
        component: Comp,
        label: "A2A",
  ...
  it("multiple legacy registers share the synthesized plugins-group", () => {
    window.Cognitwin.registerRoutes!("p1", [
      { path: "/p1", component: Comp, label: "P1", icon: "1" },
    ]);
    window.Cognitwin.registerRoutes!("p2", [
  ...
  it("new Cognitwin.menu.add coexists with legacy in plugins-group", () => {
    window.Cognitwin.registerRoutes!("cloudpaw", [
      { path: "/a2a", component: Comp, label: "A2A", icon: "🔗" },
    ]);
    window.Cognitwin.menu!.add("newPlugin", {
      id: "newPlugin.foo",
  ...
  it("legacy registerToolRender still writes to pluginSystem", async () => {
    ...
    const RenderFC = () => null;
    window.Cognitwin.registerToolRender!("p1", { my_tool: RenderFC });
