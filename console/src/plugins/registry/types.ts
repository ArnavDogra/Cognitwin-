/**
 * registry/types.ts — public shapes for the Cognitwin plugin extension registries.
 *
    ...
    dispose() {
      for (const it of d) {
        try {
          ...
          console.warn("[Cognitwin] Disposable threw on dispose:", err);
        }
      }
    },
  };
...
/** A registered route entry (added via builtinRoutes or Cognitwin.route.add). */
