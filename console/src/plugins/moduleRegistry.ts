 ...
 * 2. Plugins access and modify module exports via window.Cognitwin.modules
...
class ModuleRegistryImpl implements ModuleRegistry {
  ...
  /**
   * Get all modules (for window.Cognitwin.modules)
   */
...
// Expose to window.Cognitwin.modules (for plugin use)
// Set during initialization
if (typeof window !== "undefined") {
  if (!window.Cognitwin) {
    (window as any).Cognitwin = {};
  }

  // Use Proxy for dynamic access, ensuring plugins always get latest module state
  Object.defineProperty(window.Cognitwin, "modules", {
    get() {
